...
function dy = RW_pendulum_dynamics(y, m1, m2, l1, wr, g, u)
  ...
  Iwc= 1/2*m2*wr^2;
  dy(1,1) = y(2);
  dy(2,1) =  ((m1*l1/2+ m2*l1)*g*sin(y(1))-u)/(m2*l1^2+Ipb);
  dy(3,1) = y(4);
  dy(4,1) =  ((m1*l1/2+ m2*l1)*g*sin(y(1)))/(m2*l1^2+Ipb) +  (m2*l1^2+Ipb+Iwc)*u/(m2*l1^2+Ipb/Iwc);
endfunction
...
function [A, B] = RW_pendulum_AB_matrix(m1 , m2, l1, wr, g)
  ...
  a=m2*l1^2 + Ipb;
  b=m2*l1+m1*l1/2;
  A =[0 1 0 0;0 0 -b*g/a 0;0 0 0 1;0 0 b*g/a 0] ;
  B =[0;(a+Iwc)/a/Iwc;0;-1/a];  
endfunction
